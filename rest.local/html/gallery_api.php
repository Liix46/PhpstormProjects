...
function doGet() {
    ...
    if (is_string($DB)){ // string - means error
        ...
        sendError( [
            'code' => 507,  # Insufficient Storage (невозможно сохранить)
    ...
    try {
        ...
        logError("Select(GET): ". $ex->getMessage() . " " . $sql );
        sendError( [
            'code' => 507,  # Insufficient Storage (невозможно сохранить)
    ...
    */
    ...
    if( ! isset( $_POST[ 'pictureDescription' ] ) ) {
        sendError( [
            ...
            'text' => "Expected field(string): pictureDescription" ] ) ;
    }
    ...
    if(!str_starts_with($_FILES['pictureFile']['type'], 'image')) {
    ...
    // Secondary : moving uploaded file extension:
    $dot_pos = strrpos( $_FILES[ 'pictureFile' ][ 'name' ], '.' ) ;
    if( $dot_pos === false ) {
        // no dot in file name
        sendError( [
            'code' => 422,  # Unprocessable Entity
    ...
    $full_file_name = $saved_folder . $saved_name . $ext;
    // DB inserting
    $DB = connectDb();
    if (is_string($DB)){ // string - means error
    ...
    try {
        ...
        unlink( $full_file_name );
        //log error and exit
        logError("Insert: ". $ex->getMessage() . " " . $sql );
        sendError( [
            'code' => 507,  # Insufficient Storage (невозможно сохранить)
            'text' => "Internal error 2" ] ) ;
    }
